...
void TMTParser::Parse(bool *changedSrc, int *errPos, int *errLen, wchar_t errMsg[]) { /* abstract */ }
#if _DEBUG
void TMTParser::RemoveAltCodePath(bool *changedSrc, int *errPos, int *errLen, wchar_t error[]) { /* abstract */ }
#endif
...
typedef struct {
	...
	short minDists;
	int jumpPpemSize[maxMinDist],pixelSize[maxMinDist];
...
public:
	virtual void Parse(bool *changedSrc, int *errPos, int *errLen, wchar_t errMsg[]);
#if _DEBUG
...
private:
	...
	bool changedSrc;
	int pos,prevPos,prevPrevPos; // prevPos = position previous to starting the scanning of current symbol
	...
	int paramPos[maxParams + 1]; // +1 needed to avoid out of bounds error if at max parameters
	
	bool MakeProjFreeVector(bool haveFlag, int flagValue, bool y, ActParam *parent, ActParam child[], int children, ProjFreeVector *projFreeVector, wchar_t errMsg[]);
	...
	void Insert(int pos, const wchar_t strg[]);
...
void TMTSourceParser::Parse(bool *changedSrc, int *errPos, int *errLen, wchar_t errMsg[]) {
	Symbol cmd;
	ActParam aParam;
	short formParamNum;
	FormParam formParams[maxFPs];
	int cmdStart,actParam;
...
#if _DEBUG
void TMTSourceParser::RemoveAltCodePath(bool *changedSrc, int *errPos, int *errLen, wchar_t error[]) {
	int beginPos,endPos;
...
bool TMTSourceParser::MakeProjFreeVector(bool haveFlag, int flagValue, bool y, ActParam *parent, ActParam child[], int children, ProjFreeVector *projFreeVector, wchar_t errMsg[]) {
	...
	int i,idx = 2*(haveFlag ? 1 + flagValue : 0) + (int)y;
	...
	switch (cmd) {
		...
		case yAnchor: {
			...
			if (cvt >= 0 && cvtHint >= 0)
				...
				// if we measure perpendicular to the italic angle, and move in x, then subsequent instructions on the same knot must move aint the italic angle, and measure in y
		...
		case yDist: {
			...
			int *jSize = minDists >= 0 ? &param[base+haveCvt].jumpPpemSize[0] : NULL;
		...
		case tweakMetrics:
		default:
			swprintf(errMsg,L"Sorry, this command is no inter supported");
			break;
	}
...
void TMTSourceParser::Flag(ActParam *actParam) {
	...
	switch (this->sym) {
		case leftDir:
		case rightDir:
			actParam->type = dirFlag; actParam->numValue = (int)this->sym - (int)leftDir;
			this->GetSym();
			break;
		case italAngle:
		case adjItalAngle:
			actParam->type = angleFlag; actParam->numValue = (int)this->sym - (int)italAngle;
			this->GetSym();
			break;
		case optStroke:
		...
		case optStrokeRightBias:
			if (!this->legacyCompile)
			{
				actParam->type = strokeFlag; actParam->numValue = (int)this->sym - (int) optStroke + 1;
				this->GetSym();
			}
			break;
		case postRound:
...
void TMTSourceParser::Flag(ActParam *actParam) {
	...
	switch (this->sym) {
		...
		case adjItalAngle:
			...
			actParam->numValue = (int)this->sym - (int)italAngle;
			this->GetSym();
#ifdef VTT_PRO_SP_YAA_AUTO
			if (InitParam(this)) {
...
void TMTSourceParser::Parameter(ActParam *actParam) {
	int paramStart,localParamStart;
	short subParams;
	ActParam colorParam;
	Symbol ttvSym;
	ParamType paramType;
	int numValue,firstLocalParamStart;
	bool gotKnot[2];
...
void TMTSourceParser::ValidateParameter(ActParam *actParam) {
	...
	switch (actParam->type) { // which by now is the formal parameter type
		...
		case knotNttvOpt:
		case knotN: {
			int knot = actParam->numValue/one6;
		...
		case cvtN: {
			int cvt = actParam->numValue/one6;
...
void TMTSourceParser::GetLiteral(void) {
	...
	this->GetCh();
	if (overflow) {
		swprintf(errMsg,L"string too int (cannot be inter than %li characters)",maxAsmSize-1); this->ErrorMsg(syntactical,errMsg);
	}
}
...
void TMTSourceParser::Delete(int pos, int len) {
	if (len > 0) {
...
void TMTSourceParser::Insert(int pos, const wchar_t strg[]) {
	int len;
	
	len = (int)STRLENW(strg);
	if (len > 0) {
...
bool TMTCompile(TextBuffer *talkText, TrueTypeFont *font, TrueTypeGlyph *glyph, int glyphIndex, TextBuffer *glyfText, bool legacyCompile, int *errPos, int *errLen, wchar_t errMsg[]) {
