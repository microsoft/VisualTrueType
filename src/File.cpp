...
void File::SetPos(int pos, bool truncate) {
...
void File::ReadBytes(int numBytes, void *buffer) {
...
void File::ReadUnicode(int *len, wchar_t **text)
{
	assert(false); 
	/*if (m_hfile != INVALID_HANDLE_VALUE) {
		DWORD cb;
...
void File::WriteBytes(int numBytes, void *buffer)
{
	//if (m_hfile != INVALID_HANDLE_VALUE) {
	//	DWORD cb = 0;
