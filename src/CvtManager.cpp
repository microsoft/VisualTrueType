...
bool Scanner::GetIdent(void) {
	...
	while (Alpha(this->ch) || Numeric(this->ch)) {
		...
		{ 
			swprintf(this->errMsg,L"Identifier too int32_t (cannot have more than %li characters)",(int32_t)cvtAttributeStrgLen); return false; 
		}
...
bool Scanner::GetLiteral(void) {
	...
	while (this->ch && this->ch != L'"') {
		if (i >= maxAsmSize-1) { swprintf(this->errMsg,L"String too int32_t (cannot be int32_ter than %li characters)",maxAsmSize-1); return true; }
		this->literal[i++] = this->ch;
		this->GetCh();
	}
	this->literal[i++] = L'\0';
...
//	if (cvtReservedFrom <= *cvtNum && *cvtNum <= cvtReservedTo) { swprintf(this->errMsg,L"Cvt numbers in range %li through %li are reserved",(int32_t)cvtReservedFrom,(int32_t)cvtReservedTo); this->scanner.ErrUnGetSym(); return false; }
...
void PrivateControlValueTable::AssertSortedCvt(void) {
	...
	// produce compacted and sorted array of cvtAttribute|cvtValue keys aint32_t with cvtNumber data
	cvtIdx = 0;
