...
int32_t TT_GetLineLength( wchar_t *p, wchar_t *endP);
int32_t TT_GetLineLength( wchar_t *p, wchar_t *endP)
{
	int32_t LineLength;
...
int32_t TT_GetStringLength( wchar_t *p, wchar_t *endP);
int32_t TT_GetStringLength( wchar_t *p, wchar_t *endP)
{
...
void TT_SavePushLabel(wchar_t * CurrentPtr, short numberofLocalArgs,int32_t stringLenth,wchar_t *p,tt_PStype *PS, short * tt_error);
void TT_SavePushLabel(wchar_t * CurrentPtr, short numberofLocalArgs,int32_t stringLenth,wchar_t *p,tt_PStype *PS, short * tt_error)
{
	short i, k;
	...
	{
		if (wcsncmp( PS->ps[k]->label, p, stringLenth) == 0 && (int32_t)STRLENW(PS->ps[k]->label) == stringLenth )
		{
...
wchar_t *TT_ParseNumber( wchar_t *p, wchar_t *endP,short *Number, int32_t * SelectionLength, short * error );
wchar_t *TT_ParseNumber( wchar_t *p, wchar_t *endP,short *Number, int32_t * SelectionLength, short * error )
{
	short i, negatif;
	int32_t tempNumber,maxNum;
	wchar_t *pNumStart;
	...
	/* there is an hexadecimal number */
		...
		while ( ((*p >= L'0' && *p <= L'9') || (*p >= L'A' && *p <= L'F') || (*p >= L'a' && *p <= L'f') ) && p < endP ) 
		{
			...
			if ( *p >= L'0' && *p <= L'9' ) 
			{
				tempNumber = tempNumber * 16 + (int32_t) *p - (int32_t) L'0';
			} else if ( *p >= L'A' && *p <= L'F')
			{
				tempNumber = tempNumber * 16 + (int32_t) *p - (int32_t) L'A' + 10;
			} else if ( *p >= L'a' && *p <= L'f')
			{
				tempNumber = tempNumber * 16 + (int32_t) *p - (int32_t) L'a' + 10;
			}
			if (tempNumber > maxNum)
			{
				...
				*SelectionLength = (int32_t)((ptrdiff_t)(p - pNumStart) + 1);
				return pNumStart;
			}
			p++;
			i++;
 ...
 */
wchar_t *TT_ParsePUSHandSave(tt_PStype *ps,wchar_t *CurrentPtr,wchar_t * EOLPtr,short *argStore,short *argIdex, int32_t * SelectionLength, short * tt_error );
wchar_t *TT_ParsePUSHandSave(tt_PStype *ps,wchar_t *CurrentPtr,wchar_t * EOLPtr,short *argStore,short *argIdex, int32_t * SelectionLength, short * tt_error )
{
...
wchar_t * TT_FindLabelError(tt_PStype    *PS, tt_JrBWtype  *JrBW, tt_JRtype  *JR, tt_LabelType *Label, wchar_t * CurrentPtr, int32_t * SelectionLength, short * tt_error) ;
wchar_t * TT_FindLabelError(tt_PStype    *PS, tt_JrBWtype  *JrBW, tt_JRtype  *JR, tt_LabelType *Label, wchar_t * CurrentPtr, int32_t * SelectionLength, short * tt_error) 
{				
	short i, k;
	...
	/* look through the push on JR */
 	if ( JR->num != 0) 
 	{
		for ( k  = 0; k < JR->num; k++) {
			...
			if ( i >= Label->num ) {
				*tt_error = tt_LabelNotFound;
				*SelectionLength = (int32_t)STRLENW(JR->jr[k]->label);
	...
	/* look through the push off JR */
 	if ( JrBW->num != 0) 
 	{
		for ( k  = 0; k < JrBW->num; k++) {
			...
			if ( i >= Label->num ) {
				*tt_error = tt_LabelNotFound;
				*SelectionLength = (int32_t)STRLENW(JrBW->bw[k]->label);
			...
			if ( i >= PS->num ) {
				*tt_error = tt_LabelNotFound;
				*SelectionLength = (int32_t)STRLENW(JrBW->bw[k]->BWLabel);
				return JrBW->bw[k]->linePtr;
			}
		}
	}
	/* check if every #PUSH,Bn,Wn has a corresponding JR[], */
 	if ( PS->num != 0) 
 	{
		for ( k  = 0; k < PS->num; k++) {
			...
			if ( i >= JrBW->num ) {
				*tt_error = tt_LabelNotFound;
				*SelectionLength = (int32_t)STRLENW(PS->ps[k]->label);
...
void TT_SaveLabel(short numberofArgs,short numberofInstructions,int32_t stringLenth,wchar_t *p,tt_LabelType *Label, short * tt_error)
{
	...
	{
		if (wcsncmp( Label->lab[k]->label, p, stringLenth) == 0  && (int32_t)STRLENW(Label->lab[k]->label) == stringLenth )
		{
			*tt_error = tt_DuplicateLabel;
			return;
		}
			...
			wchar_t *BWLabelPtr,short BWstringLenth,tt_JRtype *JRList,tt_JrBWtype  *JrBW, short *aPtr,int32_t * SelectionLength, short * tt_error)
{
	...
	{
		if (wcsncmp( JRList->jr[k]->label, LabelPtr, stringLenth) == 0  && (int32_t)STRLENW(JRList->jr[k]->label) == stringLenth  )
		{
			*tt_error = tt_DuplicateLabel;
	...
	if (BWstringLenth != 0)
	{
		...
		{
			if (wcsncmp( JrBW->bw[k]->BWLabel, BWLabelPtr, BWstringLenth) == 0  && (int32_t)STRLENW(JrBW->bw[k]->BWLabel) == stringLenth )
			{
				*tt_error = tt_DuplicateLabel;
...
wchar_t * TT_ReadInstructionBooleans (wchar_t * CurrentPtr, wchar_t * EOLPtr, short InstructionIndex, unsigned short *InstructionCode, int32_t * Selectionlength, short * tt_error);
wchar_t * TT_ReadInstructionBooleans (wchar_t * CurrentPtr, wchar_t * EOLPtr, short InstructionIndex, unsigned short *InstructionCode, int32_t * Selectionlength, short * tt_error)
{
...
wchar_t * TT_DecodeDeltaP (wchar_t * CurrentPtr, wchar_t * EOLPtr, wchar_t * EndPtr, short InstructionIndex, unsigned short InstructionCode, short *deltaCount, tt_deltaPType  dArr[], int32_t * Selectionlength, short * tt_error);
wchar_t * TT_DecodeDeltaP (wchar_t * CurrentPtr, wchar_t * EOLPtr, wchar_t * EndPtr, short InstructionIndex, unsigned short InstructionCode, short *deltaCount, tt_deltaPType  dArr[], int32_t * SelectionLength, short * tt_error)
{
	short tempNum;
	wchar_t * tempP;
	...
	while ( /* [ for balance */ *CurrentPtr != L']'  && CurrentPtr <  EOLPtr) 
	{
		...
		if (TT_IsDeltaC (InstructionCode))
		{
			if (tempNum < asm_ppDescription1[6].lowestValidValue || tempNum > asm_ppDescription1[6].highestValidValue) 
			{
				...
				*SelectionLength = (int32_t) (CurrentPtr - tempP);
				return tempP;
			}
		} else {
...
void TT_GetErrorString (short ErrorNb, wchar_t * ErrorString)
{
	...
	switch (ErrorNb ) {
		...
		case tt_LabelTooLong:
			swprintf( ErrorString, L"Label too int32_t, limited to %hd character",(short) (MAXLABELLENGTH-1));
			break;
...
wchar_t * CO_ReadInstructionParameters (wchar_t * CurrentPtr, wchar_t * EOLPtr, short InstructionIndex, 
				...
				int32_t * SelectionLength,  short * co_error);
wchar_t * CO_ReadInstructionParameters (wchar_t * CurrentPtr, wchar_t * EOLPtr, short InstructionIndex, 
				co_ParameterDescriptionType co_ppDescription[], short  *argc,short *args,
				int32_t * SelectionLength,  short * co_error)
{
...
wchar_t * CO_ReadInstructionBooleans (wchar_t * CurrentPtr, wchar_t * EOLPtr, short InstructionIndex, short * RoundingCode, int32_t * Selectionlength, short * co_error);
wchar_t * CO_ReadInstructionBooleans (wchar_t * CurrentPtr, wchar_t * EOLPtr, short InstructionIndex, short * RoundingCode, int32_t * Selectionlength, short * co_error)
{
	short booleanCount, booleanShift, NBOfBooleans, found,  k;
	wchar_t * tempP;
...
wchar_t *CO_Compile(TrueTypeFont * font, TrueTypeGlyph * glyph, wchar_t *StartPtr, wchar_t * EndPtr, short *numCompositeContours, short *numCompositePoints, int32_t * SelectionLength, short * co_error);
wchar_t *CO_Compile(TrueTypeFont * font, TrueTypeGlyph * glyph, wchar_t *StartPtr, wchar_t * EndPtr, short *numCompositeContours, short *numCompositePoints, int32_t * SelectionLength, short * co_error) {
...
//	in a window that is being slided aint32_t the code. This appears to be used to determine, whether such instructions
