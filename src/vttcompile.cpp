...
bool Application::GotoGlyph(int code, bool isGlyphIndex) {
	int numGlyphs = this->font->NumberOfGlyphs(), glyphIndex, charCode;
	wchar_t errMsg[maxLineSize];
...
bool Application::CompileTalk(int* errPos, int* errLen, wchar_t errMsg[])
{
	bool legacy = false;
...
bool Application::CompileCommon(int* errPos, int* errLen, wchar_t errMsg[])
{
	bool done = false;
	bool legacy = false; 
	bool variationCompositeGuard = true; 

	int glyphIndex, binSize;
...
bool Application::CompileGlyphRange(unsigned short g1, unsigned short g2, int* errPos, int* errLen, bool quiet, wchar_t errMsg[])
{
	int glyphIndex, binSize, fromGlyph, toGlyph, numGlyphs = this->font->NumberOfGlyphs();
	bool done = false;
	unsigned char* binData;
	wchar_t tempErrMsg[maxLineSize], compErrMsg[maxLineSize];
	TextBuffer* errBuf = NULL;
	...
	for (glyphIndex = fromGlyph; glyphIndex <= toGlyph && glyphIndex < numGlyphs && done; glyphIndex++) {
		...
		if (done) {
			if (!TMTCompile(this->talk.get(), this->font.get(), this->glyph.get(), this->glyphIndex, this->glyf.get(), legacyCompile, errPos, errLen, compErrMsg)) {
				...
				this->glyf->SetText((int)STRLENW(tempErrMsg), tempErrMsg); // prevent follow-up errors
			}
		}
...
bool Application::CompileAll(int* errPos, int* errLen, bool quiet, wchar_t errMsg[])
{
	int glyphIndex, binSize, fromGlyph, fromChar, numGlyphs = this->font->NumberOfGlyphs();
	bool done;
	unsigned char* binData;
	wchar_t tempErrMsg[maxLineSize], compErrMsg[maxLineSize];
	TextBuffer* errBuf = NULL;
